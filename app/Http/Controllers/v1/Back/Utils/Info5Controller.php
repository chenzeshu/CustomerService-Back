...
class Info5Controller extends ApiController
{
    public function page($page, $pageSize)
    {
        $begin = ( $page -1 ) * $pageSize;
        $cons = Channel_info5::offset($begin)->limit($pageSize)
            ->get()->toArray();
        $total = Channel_info5::count();
        $data = [
            'data' => $cons,
            'total' => $total
        ];
    ...
    {
        $begin = ( $page -1 ) * $pageSize;
        $emp = Channel_info5::where('name', 'like', '%'.$name.'%')
            ->offset($begin)
            ->limit($pageSize)
            ->get()
            ->toArray();

        $total = Channel_info5::where('name', 'like', '%'.$name.'%')
            ->count();
