...
class Info2Controller extends ApiController
{
    public function page($page, $pageSize)
    {
        $begin = ( $page -1 ) * $pageSize;
        $cons = Channel_info2::offset($begin)->limit($pageSize)
            ->get()->toArray();
        $total = Channel_info2::count();
        $data = [
            'data' => $cons,
            'total' => $total
        ];
    ...
    {
        $data = Channel_info2::create($request->all());
    ...
    {
        $re = Channel_info2::find($id)->update($request->all());
        if($re){
            Utils::dispatch("info2");
    ...
    {
        $begin = ( $page -1 ) * $pageSize;
        $emp = Channel_info2::where('name', 'like', '%'.$name.'%')
            ->offset($begin)
            ->limit($pageSize)
            ->get()
            ->toArray();

        $total = Channel_info2::where('name', 'like', '%'.$name.'%')
            ->count();
